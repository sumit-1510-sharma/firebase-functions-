...
function App() {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        transaction.update(userRef, {
          ...
          cooldown: cooldownEnd, // ⏳ Add cooldown of 1 hour
        });
      });
  ...
  const likeImage = async (slot, userId) => {
    const slotRef = doc(db, "slots", slot);
    const likeRef = doc(db, "slots", slot, "like_ids", userId); // Store userId in likes subcollection

    try {
      await runTransaction(db, async (transaction) => {
        // Fetch only the `booked_by` field instead of the entire document
        ...
        // Add an empty document in the likes subcollection
        transaction.set(likeRef, {});

        // Increment likes count in slot document
        transaction.update(slotRef, {
          likes: increment(1),
        });

        // Increment total likes in the user document (owner of the image)
  ...
  const incrementViewCount = async (slot) => {
    ...
    try {
      ...
      return { success: true, message: "View count increased successfully!" };
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(slotsCollection, (snapshot) => {
      const imageList = snapshot.docs
        ...
        // .filter((doc) => doc.imageURL); // Filter only docs with imageURL
